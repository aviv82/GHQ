...
public class UpdateGameValidator : AbstractValidator<UpdateGameRequest>
{
    ...
    {
        RuleFor(x => x.Id).ValidateExistence<UpdateGameRequest, Game>(context).WithMessage("Game Id does not appear in the game records");
        ...
        RuleFor(x => x.DmId).ValidateExistence<UpdateGameRequest, Player>(context).WithMessage("Game's Dm Id does not appear in the player records");

        RuleForEach(x => x.Players).Must(p => context.Players.Any(cp => cp.Id == p.Id)).WithMessage("Game's Player Id does not appear in the player records");
    }
}
